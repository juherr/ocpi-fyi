...
|===
| *Authors*  | H. Ebabil (Chargemap), A. Pinto (Powerdot), T. Franquelin (Electra)
| *Version*  | 0.1 (2026-03-20)
|===
...
*OPRF*:: Oblivious Pseudorandom Functions, and by extension the protocol (https://datatracker.ietf.org/doc/rfc9497/[RFC 9497]) that makes use of these to hash MAC addresses while preserving confidentiality.
...
This specification relies exclusively on the *Oblivious Pseudorandom Function (OPRF)* primitive as defined in https://datatracker.ietf.org/doc/rfc9497/[RFC 9497]. The OPRF is used to compute a deterministic, pseudorandom identifier (the VID) from a MAC address, such that the CPO learns only the output while the eMSP's private key remains confidential.

No symmetric encryption (AEAD) or key derivation function (KDF) is mandated at the OPRF layer; confidentiality and integrity of the transport are delegated to the OCPI transport layer (TLS). See <<autocharge_oprf_suite>> for the specific OPRF suite, curve parameters, and hash function required by this specification.

[[autocharge_oprf_suite]]
=== 4.2. OPRF Suite Selection

To ensure maximum interoperability across diverse hardware and software environments, all implementations *MUST* utilize the following Oblivious Pseudorandom Function (OPRF) suite as defined in https://datatracker.ietf.org/doc/rfc9497/[RFC 9497]:
...
* *Hash-to-Group:* `SSWU` (Simplified Shallue-van de Woestijne-Ulas) method, compliant with https://datatracker.ietf.org/doc/rfc9380/[RFC 9380].
...
sequenceDiagram
    ...
    eMSP->>CPOs: PUT /vehicles/{vid} (push VID to all CPOs)
</pre>
++++
...
. *Propagation:* The eMSP confirms the link (`AUTO_CHARGE_GRANTED`) and pushes the new `+{VID, Token UID}+` entry to all CPOs in the network, making it available for future roaming.
...
| `date_created`
| xref:types.adoc#types_datetime_type[DateTime]
| no
...
|GET |n/a
|POST |n/a
|<<autocharge_cpo_put_method,PUT>> |Push new/updated Vehicle record to the CPO.
|PATCH |n/a
|DELETE |n/a
|===

[[autocharge_cpo_put_method]]
==== PUT Method
...
|<<autocharge_emsp_get_method,GET>> |Retrieve Vehicle records (VID/user pairings), paginated by `last_updated`.
|<<autocharge_emsp_post_register,POST /vehicles>> |Register a new vehicle identifier (VID) linked to a user token.
|<<autocharge_emsp_post_conflict,POST /vehicles/conflict>> |Notify the eMSP of a conflicting VID detected across multiple eMSPs.
|<<autocharge_emsp_post_oprf,POST /oprf/sign>> |Request an OPRF blind signature from the eMSP for MAC address hashing.
|PUT |n/a
|PATCH |n/a
|DELETE |n/a
|===

[[autocharge_emsp_get_method]]
==== GET Method
...
| `token_uid`
| xref:types.adoc#types_cistring_type[CiString](36)
| 1
...
| `message`
| xref:types.adoc#types_displaytext_class[DisplayText]
| ?
